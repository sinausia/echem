...
for filename in os.listdir(csv_directory):
    if filename.endswith(".csv"):
        ...
        if num_columns != 2:  # Columns time and current
            print(f"File '{filename}' has {num_columns} columns instead of 2.")
            print(f"Columns: {df.columns}")


diameter = 7  # mm
area = (np.pi * ((diameter/1000) / 2) ** 2)  # pi * (d/2)^2
...
for filename in os.listdir(csv_directory):
    if filename.endswith(".csv"):
        ...
        pH = file_to_pH.get(filename, 7.0)  # Default to pH 7.0 if file is not in the dictionary
